...
function rf2elrstelemetry.crossfirePop()
    ...
    if command and data then
        ...
        if command == CRSF_FRAME_CUSTOM_TELEM then
            ...
            local delta = (fid - rf2elrstelemetry.telemetryFrameId) & 0xFF
            ...
            setTelemetryValue(0xEE01, 0, 0, rf2elrstelemetry.telemetryFrameCount, UNIT_RAW, 0, "*Cnt", 0, 2147483647)
            setTelemetryValue(0xEE02, 0, 0, rf2elrstelemetry.telemetryFrameSkip, UNIT_RAW, 0, "*Skp", 0, 2147483647)
            -- setTelemetryValue(0xEE03, 0, 0, rf2elrstelemetry.telemetryFrameId, UNIT_RAW, 0, "*Frm", 0, 255)
        end
        return true
    end
...
function rf2elrstelemetry.run()
    ...
    for i, name in ipairs(rssiNames) do rssiSensor = system.getSource(name) end
    if rssiSensor ~= nil and rssiSensor:state() then while not ELRS_PAUSE_TELEMETRY and rf2elrstelemetry.crossfirePop() do end end
end
